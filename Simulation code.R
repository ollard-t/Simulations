...
file.exists("~/Documents/Rstudio/Simulations/BASES2/")
for(N in c(1000,3000,5000)){
  path0 <- paste0("~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats_part/N",N,"_results/",PH_val,"/Output simulations_N",N,"_",PH_val,"_",cens_val,"/")
...
simulate_iteration <- function(i, N){
  ...
  TnuFC <- -0.55
  TthetaFC <- 0
  betaZFC <- c(1, 2.8, 0.2) # c(0.6,2.6,-0.1)
  ...
  convHC <- 0
  WG.model_HC <- tryCatch(
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + 
                  ratetable(age, year, sexchara),data = data_train_HC,ratetable = slopop,dist = "weibull")
    , error = function(e) {convHC <<- 1
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + ratetable(age, year, sexchara),
                data = data_train_HC, ratetable = slopop,  dist = "exponential")
    }
  )
  
  convHR <- 0
  WG.model_HR <- tryCatch(
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + 
                  ratetable(age, year, sexchara),data = data_train_HR,ratetable = slopop,dist = "weibull")
    , error = function(e) {convHR <<- 1
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + ratetable(age, year, sexchara),
                data = data_train_HR, ratetable = slopop,  dist = "exponential")
    }
  )
  convFC <- 0
  WG.model_FC <- tryCatch(
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + 
                  ratetable(age, year, sexchara),data = data_train_FC,ratetable = slopop,dist = "weibull")
    , error = function(e) {convFC <<- 1
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + ratetable(age, year, sexchara),
                data = data_train_FC, ratetable = slopop,  dist = "exponential")
    }
  )
  convFR <- 0
  WG.model_FR <- tryCatch(
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + 
                  ratetable(age, year, sexchara),data = data_train_FR,ratetable = slopop,dist = "weibull")
    , error = function(e) {convFR <<- 1
    survivalNET(formula = Surv(times, status) ~ stage2 + stage3 + agey10 + ratetable(age, year, sexchara),
                data = data_train_FR, ratetable = slopop,  dist = "exponential")
    }
  )
  ...
  if(convHC == 1){
    logcoeffWGHC <- c(WG.model_HC$coefficients,0)
    names(logcoeffWGHC)[5] <- "log nu"
    
  }
  if(convHR == 1){
    logcoeffWGHR <- c(WG.model_HR$coefficients,0)
    names(logcoeffWGHR)[5] <- "log nu"
    
  }
  if(convFC == 1){
    logcoeffWGFC <- c(WG.model_FC$coefficients,0)
    names(logcoeffWGFC)[5] <- "log nu"
    
  }
  if(convFR == 1){
    logcoeffWGFR <- c(WG.model_FR$coefficients,0)
    names(logcoeffWGFR)[5] <- "log nu"
  ...
  for(j in strata_names){
    ...
    for(k in K){
      ...
      if (k != Kref) {
        splk  <- splinecube(timek, gammak, flex.model2.2$m_s, flex.model2.2$mpos_s[[k]])$spln
        splkP <- splinecubeP(timek, gammak, flex.model2.2$m_s, flex.model2.2$mpos_s[[k]])$spln
      } else {
        splk <- 0
        splkP <- 0
      }
    ...
    for(k in K){
      ...
      if (k != Kref) {
        splk  <- splinecube(timek, gammak, flex.model2.4$m_s, flex.model2.4$mpos_s[[k]])$spln
        splkP <- splinecubeP(timek, gammak, flex.model2.4$m_s, flex.model2.4$mpos_s[[k]])$spln
      } else {
        splk <- 0
        splkP <- 0
      }
  ...
  for(d in 1:dim(data_valid_HC)[1] ){
    hP_HC <- c(hP_HC, expectedhaz(slopop, age=data_valid_HC[d, "age"], sex=data_valid_HC[d, "sexchara"],
  ...
  for(d in 1:dim(data_valid_HR)[1] ){
    hP_HR <- c(hP_HR, expectedhaz(slopop, age=data_valid_HR[d, "age"], sex=data_valid_HR[d, "sexchara"],
  ...
  for(d in 1:dim(data_valid_FC)[1] ){
    hP_FC <- c(hP_FC, expectedhaz(slopop, age=data_valid_FC[d, "age"], sex=data_valid_FC[d, "sexchara"],
  ...
  for(d in 1:dim(data_valid_FR)[1] ){
    hP_FR <- c(hP_FR, expectedhaz(slopop, age=data_valid_FR[d, "age"], sex=data_valid_FR[d, "sexchara"],
  ...
  convergences[5,] <- convHC
  convergences[6,] <- convHR
  convergences[7,] <- convFC
  convergences[8,] <- convFR
  
  colnames(convergences)<- "conver_val"
  rownames(convergences) <- c("FM1.2", "FM1.4", "FM2.2", "FM2.4", "WG_HC", "WG_HR", "WG_FC", "WG_FR")
...
calc_indic <- function(N, iterations){
  ...
  # save.image(paste0("~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats_part/",length(iterations),"ite_",N,"ind_",date_launch,".Rdata"))
  save(list = ls(), file = paste0("~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats_part/N",N,"_results/",PH_val,"/",length(iterations),"ite_",N,"ind_",PH_val,"_",cens_val,"_",date_launch,".Rdata"))
...
calc_indic_PP <- function(N, iterations){
  ...
  # save.image(paste0("~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats_part/",length(iterations),"ite_",N,"ind_",date_launch,"_PP.Rdata"))
  save(list = ls(), file = paste0("~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats_part/N",N,"_results/",PH_val,"/",length(iterations),"ite_",N,"ind_",PH_val,"_",cens_val,"_",date_launch,"_PP.Rdata"))
...
N = 5000
path0 <- paste0("~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats_part/N",N,"_results/",PH_val,"/Output simulations_N",N,"_",PH_val,"_",cens_val,"/")
path1 <- "~/Documents/Rstudio/Simulations/BASES2/"

mclapply(iterations, simulate_iteration, N = 5000, mc.cores = detectCores() - 4)
...
while (TRUE) {
  ...
  mclapply(indic, simulate_iteration, N = 5000, mc.cores = detectCores() - 4)
  attempt <- attempt + 1
}
...
## 5000IndSTA
# path0 <- "~/Documents/Rstudio/Simulations/Simulations janvier 2026/Résultats/N5000_results/NPH/Output Simulations_N5000_NPH_HC/"
