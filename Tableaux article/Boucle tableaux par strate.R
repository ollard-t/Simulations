...
latex_table <- function(N, strata, cens_val, PH_val, PP = FALSE, yearoff = FALSE, percent = TRUE){
  ...
  xt <- xtable(whole_tout, digits = c(0,4,4,4,4,4,4,4,4,4,4))
  if(percent == TRUE){
    whole_tout[,c(1:6)] <- whole_tout[,c(1:6)]*100
  ...
  if(yearoff == TRUE){
    whole_tout_val <- whole_tout_val[,-c(1,5,9)]
  }
  xt_val <- xtable(whole_tout_val, digits = c(0,4,4,4,4,4,4,4,4,4,4))
  if(percent == TRUE){
    whole_tout_val[,c(1:6)] <- whole_tout_val[,c(1:6)]*100
    xt_val <- xtable(whole_tout_val, digits = c(0,2,2,2,2,2,2,4,4,4,2))
  }
