...
simulate_iteration <- function(i, N){
  set.seed(i)
  path0 <- paste0("~/Documents/Rstudio/Simulations/BASES2/ind",N,"NPH")
  ...
  #newchange
  sigmaC <- 75 ### 75 -> 7.5% ; 55 -> 10% ;  13.5-> 30%; 6 <- 35%, <10% d'evt au dernier temps;  5 -> ~40% ; 2.5 -> ~50% avec pas assez d'ind à risque à 10 ans 
  betaC <- c(-0.01,-0.3)
  ...
  write.table(dataC, paste0(path0,"_HC/",i,"_df",N,".csv"), sep = ';', row.names = F, col.names = T)
}
iterations <- 1:1000
...
#### 3000
mclapply(iterations, simulate_iteration, N = 3000, mc.cores = detectCores() - 4)
