...
simulate_iteration <- function(i){
  ...
  data <- read.csv(file = paste0(path1,"ind1000NPH/",i,"_df1000.csv"), sep = ";")
  Tsigma <- 12.6 
  Tnu <- -0.5
  Ttheta <- 0
  ...
  theopredval <- t(sapply(1:dim(Zdata_train)[1], FUN = funtheoval ))
  ...
  for(j in strata_names){
    ...
    colnames(Zdata_valid) <- c("stage2", "stage3", "agey10", "sex", "colon")
    ...
    theopredvalstrat <- t(sapply(1:dim(Zdata_train)[1], FUN = funtheovalstrat ))
