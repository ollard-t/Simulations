...
simulate_iteration <- function(i){
  ...
  data <- read.csv(file = paste0(path1,"ind1000_HC/",i,"_df1000HC.csv"), sep = ";")
  Tsigma <- 12.6 
  Tnu <- -0.5
  Ttheta <- 0
...
ALL_PLANN_mean_ind <- data.frame()
for(k in iterations){
  hold <- apply(read.csv(paste0(path0, "TRAIN/WHOLE/PLANN/individual_survival/",k,"_ind_PLANN.csv"), sep = ";"), mean , MARGIN = 2)
  ALL_PLANN_mean_ind <-rbind(ALL_PLANN_mean_ind, hold)
  rm(hold)
}
colnames(ALL_PLANN_mean_ind) <- colnames(ALL_PLANN)
###flex1
##2 noeuds
...
biais_PLANN_mi <- colMeans(ALL_PLANN_mean_ind[,-1] - S_theo[col( ALL_PLANN_mean_ind[,-1] )] )
RMSE_PLANN_mi <- sqrt( colMeans((ALL_PLANN_mean_ind[,-1] - S_theo[col( ALL_PLANN_mean_ind[,-1] )])^2) )
...
## PLANN mean from individuals
for(j in strata_names){
  assign(paste0("ALL_PLANN_mean_ind_", j),data.frame()) 
}
...
for(k in iterations){
  ...
  for(j in strata_names){
    ...
    hold <- unname(read.csv(paste0(path0, "TRAIN/STRATA/PLANN/individual_survival/",k,"_ind_PLANN_",j,".csv"), sep = ";") )
    
    assign(paste0("ALL_PLANN_mean_ind_",j) ,rbind(get(paste0("ALL_PLANN_mean_ind_",j)), apply(hold[,-1], mean, MARGIN =2) ) )
  }
  rm(hold)
}
...
biais_PLANN_mi_strates <- data.frame()
RMSE_PLANN_mi_strates <- data.frame()
for(j in strata_names){
  assign(paste0("biais_PLANN_mi_", j), 
         colMeans( get( paste0("ALL_PLANN_mean_ind_", j) ) - get(paste0("S_theo", j) )[col(get( paste0("ALL_PLANN_mean_ind_", j)) )] ) )
  #on concatene tous les résultats dans une dataframe pour ne pas saturer l'evt 
  biais_PLANN_mi_strates <- rbind(biais_PLANN_mi_strates, get(paste0("biais_PLANN_mi_", j)) )
  rownames(biais_PLANN_mi_strates)[dim(biais_PLANN_mi_strates)[1]] <- paste0(j)
  rm(list = paste0("biais_PLANN_mi_", j))
  
  assign(paste0("RMSE_PLANN_mi_",  j) ,
         sqrt( colMeans( ( get( paste0("ALL_PLANN_mean_ind_", j) ) - 
                             get(paste0("S_theo", j) )[col(get( paste0("ALL_PLANN_mean_ind_", j)) )] )^2 ) ) )
  RMSE_PLANN_mi_strates <- rbind(RMSE_PLANN_mi_strates, get(paste0("RMSE_PLANN_mi_", j)) )
  rownames(RMSE_PLANN_mi_strates)[dim(RMSE_PLANN_mi_strates)[1]] <- paste0(j)
  rm(list = paste0("RMSE_PLANN_mi_", j))
}

colnames(biais_PLANN_mi_strates) <- colnames(ALL_flex1.4)
colnames(RMSE_PLANN_mi_strates) <- colnames(ALL_flex1.4)
...
ALL_PLANNval_mean_ind <- data.frame()
for(k in iterations){
  hold <- apply(read.csv(paste0(path0, "VALID/WHOLE/PLANN/individual_survival/",k,"_ind_PLANNval.csv"), sep = ";"), mean , MARGIN = 2)
  ALL_PLANNval_mean_ind <-rbind(ALL_PLANNval_mean_ind, hold)
  rm(hold)
}
###flex1
##2 noeuds
...
biais_PLANNval_mi <- colMeans(ALL_PLANNval_mean_ind[,-1] - S_theoval[col( ALL_PLANNval_mean_ind[,-1] )] )
RMSE_PLANNval_mi <- sqrt( colMeans((ALL_PLANNval_mean_ind[,-1] - S_theoval[col( ALL_PLANNval_mean_ind[,-1] )])^2) )
...
## PLANN mean from individuals
for(j in strata_names){
  assign(paste0("ALL_PLANNval_mean_ind_", j),data.frame()) 
}
...
for(k in iterations){
  ...
  for(j in strata_names){
    ...
    hold <- unname(read.csv(paste0(path0, "VALID/STRATA/PLANN/individual_survival/",k,"_ind_PLANNval_",j,".csv"), sep = ";") )
    
    assign(paste0("ALL_PLANNval_mean_ind_",j) ,rbind(get(paste0("ALL_PLANNval_mean_ind_",j)), apply(hold[,-1], mean, MARGIN =2) ) )
  }
  rm(hold)
}
...
biais_PLANNval_mi_strates <- data.frame()
RMSE_PLANNval_mi_strates <- data.frame()
for(j in strata_names){
  assign(paste0("biais_PLANNval_mi_", j), 
         colMeans( get( paste0("ALL_PLANNval_mean_ind_", j) ) - get(paste0("S_theoval", j) )[col(get( paste0("ALL_PLANNval_mean_ind_", j)) )] ) )
  #on concatene tous les résultats dans une dataframe pour ne pas saturer l'evt 
  biais_PLANNval_mi_strates <- rbind(biais_PLANNval_mi_strates, get(paste0("biais_PLANNval_mi_", j)) )
  rownames(biais_PLANNval_mi_strates)[dim(biais_PLANNval_mi_strates)[1]] <- paste0(j)
  rm(list = paste0("biais_PLANNval_mi_", j))
  
  assign(paste0("RMSE_PLANNval_mi_",  j) ,
         sqrt( colMeans( ( get( paste0("ALL_PLANNval_mean_ind_", j) ) - 
                             get(paste0("S_theoval", j) )[col(get( paste0("ALL_PLANNval_mean_ind_", j)) )] )^2 ) ) )
  RMSE_PLANNval_mi_strates <- rbind(RMSE_PLANNval_mi_strates, get(paste0("RMSE_PLANNval_mi_", j)) )
  rownames(RMSE_PLANNval_mi_strates)[dim(RMSE_PLANNval_mi_strates)[1]] <- paste0(j)
  rm(list = paste0("RMSE_PLANNval_mi_", j))
}

colnames(biais_PLANNval_mi_strates) <- colnames(ALL_flex1.2)
colnames(RMSE_PLANNval_mi_strates) <- colnames(ALL_flex1.2)
...
##PLANN mean ind
biais_PLANN_mi <- rbind(biais_PLANN_mi, biais_PLANN_mi_strates)
RMSE_PLANN_mi <- rbind(RMSE_PLANN_mi, RMSE_PLANN_mi_strates)
rownames(biais_PLANN_mi)[1] <- "ALL"
rownames(RMSE_PLANN_mi)[1] <- "ALL"
rm(biais_PLANN_mi_strates)
rm(RMSE_PLANN_mi_strates)
##FLEX1
##2noeuds
...
##PLANN mean ind
biais_PLANNval_mi <- rbind(biais_PLANNval_mi, biais_PLANNval_mi_strates)
RMSE_PLANNval_mi <- rbind(RMSE_PLANNval_mi, RMSE_PLANNval_mi_strates)
rownames(biais_PLANNval_mi)[1] <- "ALL"
rownames(RMSE_PLANNval_mi)[1] <- "ALL"
rm(biais_PLANNval_mi_strates)
rm(RMSE_PLANNval_mi_strates)
##FLEX1
##2 noeuds
...
PLANN_mi_means_names <- c()
for(j in strata_names){
  PLANN_mi_means_names <- c(PLANN_mi_means_names, paste0("ALL_PLANN_mean_ind_",j))
}
PLANN_mi_means <- c(mget("ALL_PLANN_mean_ind"), mget(PLANN_mi_means_names))
rm(ALL_PLANN_mean_ind)
rm(PLANN_mi_means_names)
for(j in strata_names){
  rm(list = paste0("ALL_PLANN_mean_ind_",j) )
}
##FLEX1
##2 noeuds
...
##PLANN_mi
PLANNval_mi_means_names <- c()
for(j in strata_names){
  PLANNval_mi_means_names <- c(PLANNval_mi_means_names, paste0("ALL_PLANNval_mean_ind_",j))
}
PLANNval_mi_means <- c(mget("ALL_PLANNval_mean_ind"), mget(PLANNval_mi_means_names))
rm(ALL_PLANNval_mean_ind)
rm(PLANNval_mi_means_names)
for(j in strata_names){
  rm(list = paste0("ALL_PLANNval_mean_ind_",j) )
}
...
simulate_iteration <- function(i){
  ...
  data <- read.csv(file = paste0(path1,"ind3000_HC/",i,"_df3000HC.csv"), sep = ";")
  Tsigma <- 12.6 
  Tnu <- -0.5
  Ttheta <- 0
  betaZ <- c(0.9,2.7,0.3,-0.1,-0.1)
...
simulate_iteration <- function(i){
  ...
  data <- read.csv(file = paste0(path1,"ind5000_HC/",i,"_df5000HC.csv"), sep = ";")
  Tsigma <- 12.6 
  Tnu <- -0.5
  Ttheta <- 0
  betaZ <- c(0.9,2.7,0.3,-0.1,-0.1)
