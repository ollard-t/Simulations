...
for(N in c(1000,3000,5000)){
  path0 <- paste0("~/Documents/Rstudio/Simulations/Simulations mai 2025/RésultatsFLEX/N",N,"_results/",PH_val,"/Output simulations_N",N,"_",PH_val,"_",cens_val,"/")
...
simulate_iteration <- function(i, N){
  ...
  paramsCV <- as.data.frame(c(m1.2 = m1.2value, coeff1.2 = flex1.2.coeff, m1.4 = m1.4value, 
  ...
  timeComputation <- as.data.frame(c(time = time, N = dim(data)[1]))
  ...
  logliks <- as.data.frame(c(flex1.2 = flex1.2.loglik,
  ...
  for(j in strata_names){
    ...
    logliks[,ind_strat] <- c(loglik_s_1.2, loglik_s_1.4, loglik_s_2.2, loglik_s_2.4, get(paste0("WG_",j,".loglik")) )
  ...
  ############ enregistrement
  logliksval <- as.data.frame(c(flex1.2 = loglikval_1.2,
  ...
  for(j in strata_names){
    ...
    logliksval[,ind_strat] <- c( loglikval_s_1.2, loglikval_s_1.4, loglikval_s_2.2, loglikval_s_2.4, get(paste0("loglikval_WG_",j)) )
    ind_strat <- ind_strat+1
...
calc_indic <- function(N, iterations){
  ...
  AUC_calc_W <- function(k) {
    ...
    hold_F1.2 <- hold_F1.4 <- hold_F2.2 <- hold_F2.4 <- hold_WG <- c()
    # sort(unique(1-ind_estimP[, l + 1]))
    # unique( quantile( 1-ind_estimP[, l + 1],  probs= seq(0,1, by = 0.01)) ) #pas en dessous de 20 points 0.025
  ...
  for(l in c("F1.2","F1.4","F2.2","F2.4","WG")){
  ...
  AUC_calc_val_W <- function(k){ 
    ...
    rm(list = c(paste0("DATAValid_",k), paste0("indValidFLEX1.2_",k), paste0("indValidFLEX1.4_", k),
  ...
  AUC_calc_val_S <- function(k){ 
    ...
    for(j in strata_names){
      ...
      rm(list = c(paste0("DATAValid_",j,"_", k), paste0("indValidFLEX1.2_",j,"_", k),paste0("indValidFLEX1.4_",j,"_", k),
  ...
  save(list = ls(), file = paste0("~/Documents/Rstudio/Simulations/Simulations mai 2025/RésultatsFLEX/N",N,"_results/",PH_val,"/",length(iterations),"ite_",N,"ind_",PH_val,"_",cens_val,"_",date_launch,".Rdata"))
...
calc_indic_PP <- function(N, iterations){
  ...
  save(list = ls(), file = paste0("~/Documents/Rstudio/Simulations/Simulations mai 2025/RésultatsFLEX/N",N,"_results/",PH_val,"/",length(iterations),"ite_",N,"ind_",PH_val,"_",cens_val,"_",date_launch,"_PP.Rdata"))
...
mclapply(iterations, simulate_iteration, N = 1000, mc.cores = detectCores() - 7)
...
while (TRUE) {
  ...
  mclapply(indic, simulate_iteration, N = 1000, mc.cores = detectCores() - 7)
  attempt <- attempt + 1
}
...
mclapply(iterations, simulate_iteration, N = 3000, mc.cores = detectCores() - 7)
...
while (TRUE) {
  ...
  mclapply(indic, simulate_iteration, N = 3000, mc.cores = detectCores() - 7)
  attempt <- attempt + 1
}
...
mclapply(iterations, simulate_iteration, N = 5000, mc.cores = detectCores() - 7)
...
while (TRUE) {
  ...
  mclapply(indic, simulate_iteration, N = 5000, mc.cores = detectCores() - 7)
  attempt <- attempt + 1
}
