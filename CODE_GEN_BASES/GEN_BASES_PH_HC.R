...
path0 <- "~/Documents/Simulations/BASES/ind1000_HC/"
# path0 <- paste0(getwd(),"/")
############
...
simulate_iteration <- function(i){
  ...
  sigmaC <- 8 ### 50 -> 7.5% ;  8-> 30%; 6 <- 35%, <10% d'evt au dernier temps;  5 -> ~40% ; 2.5 -> ~50% avec pas assez d'ind à risque à 10 ans 
  
  betaC <- c(-0.01,-0.3)
  
  data$timesC <- Csim(u = runif(n = N, min=0, max=1), sigma = sigmaC, betaC = betaC, ZC = ZC) 
  table(1*(data$timesT <= data$timesC))
  ...
  write.table(data, paste0(path0,i,"_df1000HC.csv"), sep = ';', row.names = F, col.names = T)
}
...
path0 <- "~/Documents/Simulations/BASES/ind3000_HC/"
# path0 <- paste0(getwd(),"/")
############
...
simulate_iteration <- function(i){
  ...
  data$times[data$times < 10e-4] <- 10e-4
  write.table(data, paste0(path0,i,"_df3000HC.csv"), sep = ';', row.names = F, col.names = T)
}
...
path0 <- "~/Documents/Simulations/BASES/ind5000_HC/"
# path0 <- paste0(getwd(),"/")
############
...
simulate_iteration <- function(i){
  ...
  data$times[data$times < 10e-4] <- 10e-4
  write.table(data, paste0(path0,i,"_df5000HC.csv"), sep = ';', row.names = F, col.names = T)
}
